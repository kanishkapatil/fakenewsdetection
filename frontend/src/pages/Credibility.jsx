...
//     setLoading(true);
//     try {
//       const response = await API.post('/predict', { text: url }); // Using /predict as per your API list
//       setPrediction(response.data); // Expected: { prediction: "Fake", confidence: 0.85 }
//     } catch (err) {
...
//         <h1 className="text-3xl font-bold mb-8 text-cyan-400">News Credibility Verifier</h1>
//         <div className="bg-white/5 backdrop-blur-lg border border-white/10 p-8 rounded-3xl">
//           <input 
//             type="text"
//             className="w-full bg-transparent border-b-2 border-white/20 p-4 text-xl focus:border-cyan-400 outline-none mb-6"
//             placeholder="Paste news headline or full text..."
//             onChange={(e) => setUrl(e.target.value)}
//           />
//           <button onClick={checkCredibility} className="px-10 py-4 bg-cyan-600 hover:bg-cyan-500 rounded-full font-bold tracking-wide transition-all shadow-lg shadow-cyan-500/20">
//             {loading ? "Scanning Database..." : "Verify Credibility"}
//           </button>
//         </div>

//         {prediction && (
//           <div className={`mt-10 p-8 rounded-3xl border-2 transition-all ${prediction.prediction === 'Real' ? 'border-green-500/50 bg-green-500/10' : 'border-red-500/50 bg-red-500/10'}`}>
//              <h2 className={`text-6xl font-black mb-2 ${prediction.prediction === 'Real' ? 'text-green-400' : 'text-red-400'}`}>
//                {prediction.prediction}
//              </h2>
//              <p className="text-gray-400">AI Confidence: {(prediction.confidence * 100).toFixed(1)}%</p>
//           </div>
//         )}
//       </div>
//     </div>
