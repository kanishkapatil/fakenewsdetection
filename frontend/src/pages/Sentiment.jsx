...
//       setResult(response.data.sentiment); // Expected: "Positive", "Negative", or "Neutral"
//     } catch (err) {
//       console.error(err);
//     } finally {
//       setLoading(false);
...
//         <h1 className="text-3xl font-bold mb-6 bg-gradient-to-r from-pink-500 to-violet-500 bg-clip-text text-transparent">Sentiment Analysis</h1>
//         <div className="bg-white/5 backdrop-blur-xl border border-white/10 p-6 rounded-3xl shadow-2xl">
//           <textarea 
//             className="w-full h-40 bg-slate-900/50 border border-white/10 rounded-2xl p-4 focus:ring-2 focus:ring-pink-500 outline-none transition-all"
//             placeholder="Enter news text to analyze emotional tone..."
//             onChange={(e) => setText(e.target.value)}
//           />
//           <button onClick={handleAnalyze} disabled={loading} className="w-full mt-4 py-3 bg-gradient-to-r from-pink-600 to-violet-600 rounded-xl font-bold hover:opacity-90 transition-all">
//             {loading ? "Analyzing..." : "Analyze Sentiment"}
//           </button>
//         </div>

//         {result && (
//           <div className="mt-8 p-6 bg-white/5 border border-white/10 rounded-3xl text-center animate-bounce">
//             <span className="text-gray-400 block mb-2 uppercase text-xs tracking-widest">Detected Tone</span>
//             <h2 className={`text-5xl font-black ${result === 'Positive' ? 'text-green-400' : result === 'Negative' ? 'text-red-400' : 'text-yellow-400'}`}>
//               {result}
//             </h2>
//           </div>
//         )}
