...
def get_prediction_logic(text):
    """
    Unified prediction logic shared by both the Text Predictor and DocIntel.
    """
    if not text or not text.strip():
        return {"prediction": "Unknown", "confidence": 0, "top_keywords": []}

    try:
        # 1. Detect Language & Translate (Reuse your existing logic)
        source_lang = detect(text)
        analysis_text = text
        if source_lang != 'en':
            analysis_text = GoogleTranslator(source='auto', target='en').translate(text)

        # 2. ML Prediction (Uses the model/vectorizer loaded at top of predict.py)
        X = vectorizer.transform([analysis_text])
        pred = int(model.predict(X)[0])
        prob = model.predict_proba(X)[0]
        ...
        # 4. Google Fact Check Integration
        google_result = get_google_fact_check(analysis_text)
        prediction = "Real" if pred == 1 else "Fake"
        confidence = round(float(np.max(prob)), 2)

        if google_result["found"]:
            # Override if fact checkers have a verdict
            if any(word in google_result["rating"].lower() for word in ["false", "fake", "misleading", "incorrect"]):
                prediction = "Fake"
                confidence = 1.0
        ...
        return {"prediction": "Error", "confidence": 0, "top_keywords": []}